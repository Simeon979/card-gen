...
const generateCard = (option = {}) => {
    ...
    if (options.hasOwnProperty("startsWith")) {
        if (options.startsWith.length > 10 || options.startsWith.length < 1) throw new Error("startsWith must consist of between 1 and 10 digits");
        if (!parseInt(options.startsWith, 10)) throw new Error("startsWith must consist of digits only");
        card += options.startsWith;
    } else if (options.hasOwnProperty("includeSequence")) {
        if (options.includeSequence.length > 10 || options.includeSequence.length < 1) throw new Error("includeSequence must consist of between 1 and 10 digits");
    ...
    } else if (options.hasOwnProperty("endsWith")) {
        if (options.endsWith.length > 10 || options.endsWith.length < 1) throw new Error("endsWith must consist of between 1 and 10 digits");
...
const normalizeOptions = (options) => {
    const result = {};
    if (options.hasOwnProperty("startsWith")) result.startsWith = options.startsWith + "";
    if (options.hasOwnProperty("network")) result.network = options.network + "";
    if (options.hasOwnProperty("issuer")) result.issuer = options.issuer + "";
    if (options.hasOwnProperty("endsWith")) result.endsWith = options.endsWith + "";
    if (options.hasOwnProperty("verve")) result.verve = !!options.verve;
    if (options.hasOwnProperty("includeSequence")) result.includeSequence = options.includeSequence + "";
    return result;
}
